...
local function register_storage_store(_, image, _, tmp_exp_path, _, _, _, _)
  msgout(gettext.dgettext("dtMediaWiki", ("register_storage_store: exporting the following image:")) .. tmp_exp_path .. '(' .. image.title .. ')')
  for _, tag in pairs(dt.tags.get_tags(image)) do
    if tag == 0 then
...
local function register_storage_initialize(_, _, images, _, extra_data)
  local out_images = {}
  for _, img in pairs(images) do  -- BUG? images contain "0" value (with key 3 in tested instance) instead of dt_lua_image_t
	if type(img) == 'number' then
		msgout(gettext.dgettext("dtMediaWiki", ("BUG: dt.register_storage.initialize sent a number instead of dt_lua_image_t in images table: ")) .. img)
