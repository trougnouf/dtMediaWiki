...
local function get_alt_images(image)
  local alts = {}
  for _, tag in pairs(dt.tags.get_tags(image)) do
  ...
  local hardcoded_extension = '.jpg'
  local altcode = {"<gallery showfilename=yes>"}
  for _, image in ipairs(dt.collection) do
    for _,altimg in pairs(alts) do
      if string.find(image.filename, altimg) then
        table.insert(altcode, make_image_name(image, hardcoded_extension))
      end
    end
  end
...
local function make_image_page(image)
  ...
  table.insert(imgpg, '|other versions = ' .. get_alt_images(image))
  table.insert(imgpg, "}}")
